...
Chrome do Android, contra o servidor.
...
A causa é o endereço: o dwnvr é acessado em `http://servidor:8080`, e a **Web
...
- Um certificado do Tailscale só vale para quem entra pelo tailnet. Quem abre
  `http://servidor:8080` pela LAN continua em contexto inseguro, então na prática
...
O caminho mais barato é o Tailscale, que já roda no servidor com MagicDNS ligado
(sufixo `exemplo.ts.net`, confirmado em 19/08/2026) e hoje **não tem nenhum
serve configurado**:

```sh
# no servidor, depois de habilitar HTTPS Certificates no admin do tailnet
tailscale serve --bg 8080
# passa a atender em https://servidor.exemplo.ts.net, com certificado de verdade
```
