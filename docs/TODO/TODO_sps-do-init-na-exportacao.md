...
| cam_zeta, cam_eta | AVC | 1280x720 | 1280x720 | ok |
...
gravado. Uma primeira passada com `ffprobe` deu a `cam_epsilon` como correta.
...
câmeras - `4edbc50d8e70` é byte a byte idêntico em teta/alfa/beta/iota/
delta, `e38cb0530c62` em teta/gama (as duas com flac), `ead03cc7ac5a` em
epsilon/zeta/eta. O init é função de (codec, áudio) e de mais nada.
...
E uma comparação de prioridade que pesa mais que tudo acima: **a `cam_iota`
gravando 640x360 é um problema maior que este metadado inteiro.** Se ela é a
câmera que vê quem chega, a fonte no `go2rtc.yaml` provavelmente
